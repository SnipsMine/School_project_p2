    ...
    for obj in ANIMATION_OBJECTS:
        ...
        if molecule_data[0] and not molecule_data[1]:
            ...
            molecule ={"molecule": mol,
                       "reset": [0, [mol.atoms[val] for val in range(len(mol.atoms))]],
                       }
...
def calculate_radians(frames, ends, step = 1):
    """Calculates how many radians must be moved per step"""
    time = frames[1] - frames[0]
    radians_list = []

    for end in ends:
        radians = end / time * step - frames[0]
...
def rotate_objects(obj, step, mother=False):
    ...
    reset_num = MOLECULES[obj]["reset"][0]
    reset_atoms = MOLECULES[obj]["reset"][1]

    molecule = MOLECULES[obj]["molecule"]
    ...
    for frame in range(len(rotate_frames_data)):
        ...
        if frame != 0 and step in [val for val in range(rotate_frames_data[frame-1]+1,
            ...
            # not step > rotate_frames_data[reset_num] and 

            #print(reset_num == frame - 1)
            #if not reset_num == frame - 1:
            #    update_molecule_reset(obj, frame)
            ...
            if mother:
                print(molecule)
                molecule.atoms = reset_atoms
                
                radians = calculate_radians([rotate_frames_data[frame-1], rotate_frames_data[frame]],
                                             rotate_endpos_data[frame][1], step
                                             )

                molecule.rotate([1, 1, 1], [radians[0]*-1, radians[1]*-1, radians[2]*-1])
                print(molecule)
            else:
                print(molecule.atoms == reset_atoms)
                molecule.atoms = reset_atoms
                print(molecule.atoms == reset_atoms)
...
def update_molecule_reset(obj, frame):
    rotate_frames_data = ANIMATION_OBJECTS[obj]["keyframe_rotation_frames"]
    rotate_endpos_data = ANIMATION_OBJECTS[obj]["keyframe_rotation"]
    ...
    print("(reset) {}: updated from {} to {}".format(obj, reset_num, reset_num +1))

    rotate_objects(obj, rotate_frames_data[reset_num+1])
    move_objects(obj, rotate_frames_data[reset_num+1])

    reset_atoms = MOLECULES[obj]["molecule"].atoms.copy()
...
def main():
    ...
    pypovray.render_scene_to_png(make_frame, range(74, 700))
    return 0
